...
reduced_embeddings = reduce_dimensions(quantized_embeddings, n_components=100, n_neighbors=25)
...
df.to_csv("FINAL_CLEAN_embeddings_contrarian.csv", index=False)
...
for label in unique_labels:
    ...
    fig.add_trace(go.Scatter3d(
        ...
        text = [f"Index: {df.index[i]}<br>Domain: {df.iloc[i]['domain']}<br>Title: {df.iloc[i]['title'] if pd.notna(df.iloc[i]['title']) else df.iloc[i]['title']}<br>Cluster: {label}<br>Excerpt: {format_content(df.iloc[i]['clean_content'][:900] if pd.notna(df.iloc[i]['clean_content']) else df.iloc[i]['content'][:900])}" 
    for i in indices],
        hoverinfo='text',
...
fig.write_html('umap_3d_cluster_contrarian_final.html')
